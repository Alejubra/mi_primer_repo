...
- Carga (lectura) de datos (los datos pueden venir de varias fuentes, un csv, un excel, una base de datos, stora accaount nube, bucket, entre otros)
- Limpiar nombres de las columnas (variables, de queson, cual es el contecto, que tipo de variables se tiene,entender los datos en general, a veces los nombres de las variables no son representativos, o tienen caracteres especiales, vienen columnas vacias, entre otros)
- Identificación de observaciones duplicadas y/o de duplicación de datos (a veces los datos pueden venir desordenados, sucios, las observaciones pueden venir doble o triple o pueden haber filas que se repitan, por ende es importante desacerse de esos duplicados)
- Detección y tratamiento de datos faltantes 
- Transformación de variables
- Creación de nuevas variables (por ejemplo utilizando la función mutate de dplyr)
...
La recomendación para unir el nombre de una variable que viene dividida con espacios es por medio de un _ (guión bajo).La función clean names de janitor de manera automática hace ese tipo de ajustes.
...
## Identificación de duplicados y deduplicación (desacerse de filas o columnas redundantes)
...
esa combinación de valores duplicados (es decir cuantas veces se repite al 100% esa fila).

Esta función lo que devuelve es una tabla con los valores duplicados al 100% en todas las columnas.
...
Además, recordar, que la función get_dupes () identifica que registros están duplicados, entonces para eliminarlos se usa la función distinct ().

```{r}
filas_duplicadas_distintas <- datos_limpios |>  
  janitor::get_dupes() |> 
  distinct()
```
...
```{r}
# Duplicados sin considerar la columna recordID (los :: es para decir que dicha función le pertenece a x ,libreria, en este caso la libreria es janitor)
...
puede ser de gran valor, como para hacer reortes que indiquen que están sucediendo duplicados, para que se tomen medidas correctivas por ejemplo dentro de una app.
...
de dplyr (lo que hace es mutar, transformar, todas las columnas y ordena: aplique lo siguiente...), como se muestra a continuación:

Esta funcion lo que hace es colocar un NA si hay un espacio vacío.
...
Lo que significa esa línea de código anterior es: si alguna observación es un espacio vacio, entonces transformela en NA.

Los NAs son más sencillos de manejar en RStudio que los espacios vacios. 
...
Lo que significa las siguientes líneas de código es:
cuando presentó sintomas = a N transformela en NO, 
Sí presentó sintomas esta entre Y y Yes transformelo en Si,
TRUE: y todo lo que no caiga en alguno de estos dos casos dejelo igual.

TRUE = mantiene iguales los que no cumplen las condiciones.
...
datos_infeccion_3 <- datos_infeccion_2 |> 
  mutate(presento_sintomas = case_when(presento_sintomas == "N" ~ "No",
                                       ...
                                       TRUE ~ presento_sintomas)) 
```



En caso de necesitar aplicar otro mutate al código anterior, solo se agrega un pipe despues del )) y se puede continuar el ejecicio anterior agregando esto:
|>  
mutate(razon_encuentro = case_when(razon_encuentro == "No info" ~ "Sin información",
TRUE ~ razon_encuentro))
